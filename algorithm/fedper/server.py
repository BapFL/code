    ...
    def federate(self):
        ...
        for i in tqdm(range(self.config.num_rounds)):
            ...
            r = {'mnist': [50],
                 'femnist': [100],
                 'fashionmnist': [200],
                 'cifar10': [500]}[self.config.dataset]
            ...
            if i + 1 in r:
                ...
                if self.config.clip_grad:
                    if self.config.norm_threshold >= 10000.0:
                        ...
                        assert self.config.norm_threshold in [0.1, 0.3, 0.5]
                        path = f'{dir_name}/{i + 1}-GNC({self.config.norm_threshold})'
                elif self.config.median:
                    path = f'{dir_name}/{i + 1}-median'
                elif self.config.multi_krum:
                    path = f'{dir_name}/{i + 1}-multikrum(m{self.config.M}-k{self.config.K})'
                elif self.config.trimmed_mean:
                    path = f'{dir_name}/{i + 1}-trimmedmean({self.config.trimmed_beta})'
                else:
                    path = ""
                if os.path.exists(path):
                    print("pretrained model has been generated, sure to overwrite？")
                    exit(0)
                if not os.path.isdir(path):
                    os.makedirs(path)
                torch.save(self.model, f'{path}/server.pt')
                for c in self.clients:
                    torch.save(c.model, f'{path}/client-{c.user_id}.pt')
