...
class CLIENT:
    ...
    def train(self, round_th, partner=None, partner_model=None):
        ...
        # !!! not name.startswith -> name in self.share_layers
        meta_params = [param for name, param in model.named_parameters() if param.requires_grad and name in self.share_layers]
        ...
        for it in range(self.config.local_iters):
            ...
            if self.is_attacker and (round_th+1) >= self.config.start_attack_round:
                ...
                clean_x, clean_y, poison_x, poison_y = x[poison_count:], y[poison_count:], x[:poison_count], y[:poison_count]
                clean_x, clean_y, poison_x, poison_y = clean_x.to(self.device), clean_y.to(self.device), poison_x.to(self.device), poison_y.to(self.device)
                ...
                with higher.innerloop_ctx(model, optimizer, copy_initial_weights=False) as (fnet, diffopt):
                    ...
                    partner_poison_x, partner_poison_y, poison_count = get_poison_batch(images=partner_x, targets=partner_y, evaluation=False, config=self.config)
                    partner_poison_x, partner_poison_y = partner_poison_x.to(self.device), partner_poison_y.to(self.device)
                    partner_model.to(self.device)
                    partner_model.eval()
                    partner_poison_embedding = partner_model.conv(partner_poison_x)
                    partner_poison_logits = fnet.clf(partner_poison_embedding)
    ...
    def performance_test(self):
        ...
        attack_test_samples, ASR, attack_loss = self.test(model=self.model,data_loader=self.asr_test_loader, eval_asr=True)
        self.stats.update({
    ...
    def test(self, model=None, data_loader=None, eval_asr=False):
        ...
        with torch.no_grad():
            for step, (data, labels) in enumerate(data_loader):
                if eval_asr:
                    data, labels, poison_count = get_poison_batch(images=data, targets=labels, evaluation=True, config=self.config)
