...
from algorithm.Gen_BapFL_FedPer.client import CLIENT
...
class SERVER:
    ...
    def federate(self):
        ...
        dir_name = f'./pretrain_models/' \
                   f'{self.config.dataset}/' \
                   f'{self.config.alpha}/' \
                   f'{self.config.model}/' \
                   f'{self.config.num_share_layers}'
        # dir_name = f'./pretrain_models_with_defend/{self.config.norm_threshold}/' \
        #            f'{self.config.dataset}/' \
        #            f'{self.config.alpha}/' \
        #            f'{self.config.model}/' \
        #            f'{self.config.num_share_layers}'
        if pretrain_rounds > 0:
            ...
            pretrain_model_state = torch.load(f'{dir_name}/{pretrain_rounds}/server.pt',
                                              map_location=self.device).state_dict()
            pretrain_model_keys = list(pretrain_model_state.keys())
            model_keys = list(self.model.state_dict().keys())
            for key, value in self.model.named_parameters():
                corr_id = pretrain_model_keys[model_keys.index(key)]
                value.data = pretrain_model_state[corr_id].data

            for c in self.clients:
                c_pretrain_model_state = torch.load(f'{dir_name}/{pretrain_rounds}/client-{c.user_id}.pt',
                                                    map_location=self.device).state_dict()
                c_pretrain_model_keys = list(c_pretrain_model_state.keys())
                c_model_keys = list(c.model.state_dict().keys())
                for key, value in c.model.named_parameters():
                    corr_id = c_pretrain_model_keys[c_model_keys.index(key)]
                    value.data = c_pretrain_model_state[corr_id].data
        ...
        for i in tqdm(range(self.config.num_rounds)):
            ...
            for c in self.selected_clients:
                if c.is_attacker:
                    partners = deepcopy(self.config.attackers)
                    partners.remove(c.user_id)
                    partner_id = random.choice(partners)
                    partner = self.clients[partner_id]
                    partners_info.update({c.user_id: [partner, deepcopy(partner.model)]})
                else:
                    partners_info.update({c.user_id: [None, None]})
            for k in range(len(self.selected_clients)):
                c = self.selected_clients[k]
                c.set_params(deepcopy(self.model_params))
                train_samples_num, c_model_params, loss = c.train(round_th=i, partner=partners_info[c.user_id][0], partner_model=partners_info[c.user_id][1])
                self.clients_model_params.append((train_samples_num, c_model_params))
            model_params = fed_average(self.clients_model_params,
                                       threshold=self.config.norm_threshold,
                                       old_params=deepcopy(self.model.cpu().state_dict()),
                                       clip_grad=self.config.clip_grad,
                                       dp_noise=self.config.dp_noise,
            ...
            self.clients_model_params = []
        if self.config.save_poisoned_models:
            dir_name = f'./poisoned_models/{self.config.algorithm}/' \
                       f'{self.config.dataset}/' \
                       f'{self.config.alpha}/' \
                       f'{self.config.model}/' \
                       f'{self.config.num_share_layers}'
            ...
            torch.save(self.model, f'{dir_name}/server.pt')
            for c in self.clients:
                torch.save(c.model, f'{dir_name}/client-{c.user_id}.pt')
