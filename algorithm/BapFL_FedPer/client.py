...
class CLIENT:
    def __init__(self, user_id, train_loader, test_loader, asr_test_loader, config):
        ...
        self.share_layers = [k for k in keys if
                             2 * self.config.private_bottom <= keys.index(k) <= (2 * self.config.num_share_layers - 1)]
