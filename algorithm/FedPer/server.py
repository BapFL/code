...
import torch
from utils.utils import setup_seed, fed_average, parameters_distance
from utils.set_md import select_model, setup_datasets, avg_metric
from algorithm.FedPer.client import CLIENT
...
class SERVER:
    ...
    def federate(self):
        ...
        # dir_name = f'./pretrain_models_with_private_bottom/{self.config.private_bottom}/' \
        ...
        for i in tqdm(range(self.config.num_rounds)):
            ...
            if self.config.pretrain and (i == 0 or (i + 1) % 100 == 0):
                if not os.path.isdir(f'{dir_name}/{i+1}'):
                    os.makedirs(f'{dir_name}/{i+1}')
                torch.save(self.model, f'{dir_name}/{i+1}/server.pt')
                for c in self.clients:
                    torch.save(c.model, f'{dir_name}/{i+1}/client-{c.user_id}.pt')
            self.clients_model_params = []
