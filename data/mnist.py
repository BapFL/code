...
def dirichlet_partition_data(train_dataset, test_dataset, num_users=100, num_classes=10, alpha=0.5,
    ...
    for k in range(num_classes):  # 10个类别
...
def get_mnist_dataloaders(num_users=100, batch_size=10, alpha=0.5, poison_swap_label=2):
    setup_seed(24)
...
if __name__ == '__main__':
    train_loaders, test_loaders, attack_test_loaders = get_mnist_dataloaders(num_users=50, batch_size=64, alpha=0.05,
                                                                             poison_swap_label=2)
    # 5: (7, 8)
    # 9: (0, 9)
    # 31: (1, 3)
