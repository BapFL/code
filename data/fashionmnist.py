...
def dirichlet_partition_data(train_dataset, test_dataset, num_users=100, num_classes=10, alpha=0.5, poison_swap_label=2):
    train_classes_dict = {}
...
def get_fashion_mnist_dataloaders(num_users=100, batch_size=10, alpha=0.5, poison_swap_label=2):
    setup_seed(24)
    ...
    user_ids = dirichlet_partition_data(train_dataset=train_dataset, test_dataset=test_dataset, num_users=num_users, num_classes=10, alpha=alpha, poison_swap_label=poison_swap_label)
    ...
    for i in range(num_users):
        train_loader = torch.utils.data.DataLoader(train_dataset,
                                                   ...
                                                   sampler=torch.utils.data.sampler.SubsetRandomSampler(user_ids[i]['train']),
                                                   pin_memory=True,
                                                   num_workers=0,
                                                   drop_last=False)
        ...
        test_loader = torch.utils.data.DataLoader(test_dataset,
                                                   batch_size=batch_size,
                                                   sampler=torch.utils.data.sampler.SubsetRandomSampler(user_ids[i]['test']),
                                                   pin_memory=True,
                                                   num_workers=0)
        test_loaders.append(test_loader)
        attack_test_loader = torch.utils.data.DataLoader(test_dataset,
                                                  batch_size=batch_size,
                                                  sampler=torch.utils.data.sampler.SubsetRandomSampler(user_ids[i]['attack_test']),
                                                  pin_memory=True,
                                                  num_workers=0)
...
def count(dataloader):
    ...
    labels = dataloader.dataset.targets[indices]
    unique_labels = labels.unique()
    for l in unique_labels:
        print(l, ": ", np.where(labels == l)[0].shape[0])
if __name__ == '__main__':
    train_loaders, test_loaders, attack_test_loaders = get_fashion_mnist_dataloaders(num_users=50, batch_size=64, alpha=0.05, poison_swap_label=2)
    # 5: (7, 8)
    # 9: (0, 9)
    # 31: (1, 3)
    # 37: (2, 6)
    # 38: (4, 5)
    # for step, (x, y) in enumerate(train_loaders[0]):
    #     print(y)
    #     break
    # for step, (x, y) in enumerate(test_loaders[0]):
    #     print(y)
    #     break
    for c_id in range(50):
        print(c_id)
        count(train_loaders[c_id])